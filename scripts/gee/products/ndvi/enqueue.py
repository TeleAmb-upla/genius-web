...
def enqueue_ndvi_exports(
    *,
    ...
    tables_run = plan.run if tables_run_override is None else tables_run_override
    ...
    if want("raster"):
        ...
        if missing_assets:
            ...
            if skip_yearly:
                messages.append(
                    "Omitido: raster de tendencia (--include-yearly). Con "
                    f"--include-yearly: ``NDVI_Yearly_Trend.tif`` → Drive "
                    f"«{paths.DRIVE_RASTER_TREND}» (solo tendencia; anuales en "
                    f"«{paths.DRIVE_RASTER_YEARLY}»)."
                )
            else:
                lfy_tr = ym_lib.effective_yearly_export_year(ic_r)
                need_trend = plan.run or incremental.should_refresh_trend_raster(ic_r)
                if need_trend:
                    reason = (
    ...
    if want("geojson"):
        if not skip_yearly and tables_run and refresh_yearly_products:
            _add_tasks(
                drive,
            ...
            ran_derivative = True
        elif not skip_yearly and tables_run:
            messages.append(
