import { createYearSelector, positionYearSelector } from './aod_z_m/utils_z_m_y.js';
import { createMonthSelector, positionMonthSelector } from './aod_z_m/utils_z_m_m.js';
import { LayersControl } from '../control.js';
import { updateMapLayerYear, updateMapLayerMonth } from './aod_z_m/layer_z_m.js';
import { createYearLegend, createMonthLegend } from './aod_z_m/legend.js';
import {map_trend, createTrendLegend} from './aod_z_m/trend_m.js';
...
export async function map_aod_m() {
  const [defaultBeforeYear, defaultAfterYear] = getDefaultYearPair('aod');

    const container = document.getElementById('p25');
    container.innerHTML = `
      <div id="before_m_aod" style="width: 100%; height: 100%;"></div>
      <div id="after_m_aod" style="width: 100%; height: 100%;"></div>
<div id="title" class="map-title">
      AOD Estadística Zonal (Manzana)
      </div>
    `;
  
    const beforeMap = new maplibregl.Map({
      container: 'before_m_aod', // Contenedor ajustado a 'before_m_aod'
      style: {
          version: 8,
          sources: {
              carto: {
  ...
  const afterMap = new maplibregl.Map({
      container: 'after_m_aod', // Contenedor ajustado a 'after_m_aod'
      style: {
          version: 8,
          sources: {
              carto: {
...
function removeAllLayers(map) {
     ...
     async function setMode(mode) {
      ...
      setCompareSingleMapMode({
          container,
          beforeSelector: '#before_m_aod',
          afterSelector: '#after_m_aod',
          beforeMap,
          afterMap,
          enabled: mode === 'trend',
      });
     ...
     window.compareInstance = new maplibregl.Compare(beforeMap, afterMap, '#p25', {
       mousemove: false,
