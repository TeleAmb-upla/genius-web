...
# Mismo criterio que scripts/repo/rasters/convert_illumination.py (mapa index2 / Leaflet).
MAX_WEBP_SIDE = 4096
WEBP_QUALITY = 80
...
def main() -> int:
    ...
    with rasterio.open(src_tif) as ds:
        ...
        w, h = img.size
        scale = min(MAX_WEBP_SIDE / w, MAX_WEBP_SIDE / h, 1.0)
        if scale < 1.0:
            nw, nh = max(1, int(w * scale)), max(1, int(h * scale))
            img = img.resize((nw, nh), Image.Resampling.NEAREST)
            print(f"  Downscale clasificación: {w}x{h} → {nw}x{nh} (NEAREST)")
        img.save(out_webp, "WEBP", quality=WEBP_QUALITY, lossless=False)
