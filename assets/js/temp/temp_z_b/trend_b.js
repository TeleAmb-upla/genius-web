...
import { lstBarrioPopupHtmlTrend } from '../lst_zonal_explorer.js';
...
export async function map_trend(map) {
    ...
    const domain = legendDomain('lst', 'zonalBarrio', 'trend');
    ...
    map.on('click', 'generic-trend-layer', (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(lstBarrioPopupHtmlTrend(properties));
...
export function createTrendLegend() {
    ...
    title.textContent = geniusTitleForProduct('Tendencia LST (°C) Barrios', 'lst');
    title.className = 'map-legend-panel__title';
    fillZonalTrendLegendPanel(legendContent, title, 'lst', 'zonalBarrio', {
        steps: 7,
        decimals: 3,
    });
