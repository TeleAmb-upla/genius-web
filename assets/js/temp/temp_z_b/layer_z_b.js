...
import {
    lstBarrioPopupHtml,
    lstBarrioPopupHtmlMonthly,
} from '../lst_zonal_explorer.js';
        ...
        data.features.forEach(feature => {
            if (feature.properties && feature.properties.LST_mean !== undefined) {
                feature.properties.color = currentMode === 'yearly' 
                    ? ToColorYear_z_b(feature.properties.LST_mean)
                    : ToColorMonth_z_b(feature.properties.LST_mean);
            }
        });
        return data;
    } catch (error) {
...
export async function updateMapLayerYear(map, sourceId, layerId, year) {
    const url = resolveAssetUrl(`assets/data/geojson/LST/LST_Yearly_ZonalStats/LST_Yearly_ZonalStats_Barrios/LST_Yearly_ZonalStats_Barrios_${year}.geojson`);
    ...
    map.on('click', layerId, (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(lstBarrioPopupHtml(properties));
...
export async function updateMapLayerMonth(map, sourceId, layerId, month) {
    const url = resolveAssetUrl(`assets/data/geojson/LST/LST_Monthly_ZonalStats/LST_Monthly_ZonalStats_Barrios/LST_Monthly_ZonalStats_Barrios_${month}.geojson`)
    ...
    map.on('click', layerId, (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(lstBarrioPopupHtmlMonthly(properties));
        geniusPrepareExclusiveGeoPopup(popup);
        popup.addTo(map);
    });
