...
export async function map_trend(map) {
    try {
        // Leer el archivo  
        const response = await fetch(resolveAssetUrl('assets/data/raster/SO2/SO2_Trend/SO2_Yearly_Trend.tif'));
...
export function createSTLegendSVG() {
    ...
    const valueLabels = Array.from({ length: steps }, (_, i) => {
        const value = (domain[0] + i * (domain[1] - domain[0]) / (steps - 1)).toFixed(0); // Redondear el valor
        const nextValue = (domain[0] + (i + 1) * (domain[1] - domain[0]) / (steps - 1)).toFixed(0); // Valor siguiente para el rango
        ...
        if (i === 0) {
            ...
            return `<text x="60" y="${yPosition}" font-size="10" font-family="Arial">&gt;170</text>`; // Último bloque para >7.00
