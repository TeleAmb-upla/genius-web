...
function valueToSTColor(value) {
    ...
    const spec = trendLegendSpec('so2', 'raster');
    if (!spec) return null;
...
export async function map_trend(map) {
    try {
        const response = await fetch(
            resolveAssetUrl('assets/data/raster/SO2/SO2_Trend/SO2_Yearly_Trend.tif'),
        );
...
export function createSTLegendSVG() {
    const spec = trendLegendSpec('so2', 'raster');
    const title = `SO<tspan baseline-shift="sub">2</tspan> — Tendencia${geniusYearSpanSuffix('so2')}`;
    if (!spec) {
        return `<svg class="map-legend-svg" width="200" height="80" xmlns="http://www.w3.org/2000/svg"><text x="5" y="22" font-size="14" font-family="Arial" font-weight="bold">${title}</text></svg>`;
    }
    const steps = 9;
    ...
    return `
        <svg class="map-legend-svg" width="200" height="${totalHeight + 80}" xmlns="http://www.w3.org/2000/svg">
            <text x="5" y="22" font-size="14" font-family="Arial" font-weight="bold">${title}</text>
            ${legendItems}
            ${valueLabels}
        </svg>
    `;
