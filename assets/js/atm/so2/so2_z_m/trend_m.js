...
function valueToSTColor(value) {
    const spec = trendLegendSpec('so2', 'zonalManzana');
...
export async function map_trend(map) {
    const geojsonUrl = resolveAssetUrl('assets/data/geojson/SO2/SO2_Yearly_ZonalStats/SO2_Yearly_ZonalStats_Manzanas/Trend_SO2_ZonalStats_Manzanas.geojson');
...
export function createTrendLegend() {
    ...
    title.innerHTML = `Tendencia SO<sub>2</sub> Manzanas${geniusYearSpanSuffix('so2')}`;
    fillZonalTrendLegendPanel(legendContent, title, 'so2', 'zonalManzana', {
        steps: 7,
        decimals: 2,
    });
