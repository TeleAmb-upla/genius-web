...
export async function g_a_no2() {
    ...
    d3.select("#p29").selectAll("*").remove();

    // Append the svg object to the div with id "p29"
    var svg = d3.select("#p29")
        .append("svg")
    ...
    // Parse the Data
    const data = await d3.csv("/assets/csv/NO2_Anual_Regional.csv");

    // Format the data
    data.forEach(d => {
        d.Year = +d.Year;           // Convert Year to number
        d.NO2_median_fixed = +d.NO2_median_fixed;    // Convert NO2_median_fixed to number
    });
