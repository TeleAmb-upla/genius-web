...
let legendDiv = null;  // Variable global para almacenar el elemento del título
...
export async function map_aod_p() {
    ...
    updateCenteredTitle(currentMap, "AOD Pixel Distrito Urbano");
    ...
    const initialLeftLayerKey = "AOD 2019";
    const initialRightLayerKey = "AOD 2023";
    ...
    document.getElementById('yearLeft').addEventListener('change', function() {
        ...
        const newLeftLayer = LayersYearth[`AOD ${selectedYearLeft}`];
        if (newLeftLayer) {
    ...
    document.getElementById('yearRight').addEventListener('change', function() {
        ...
        const newRightLayer = LayersYearth[`AOD ${selectedYearRight}`];
        if (newRightLayer) {
    ...
    document.getElementById('monthLeft').addEventListener('change', function() {
        ...
        const newLeftLayer = Layersmonth[`AOD ${selectedMonthLeft}`];
        if (newLeftLayer) {
            if (leftLayer) currentMap.removeLayer(leftLayer);
            leftLayer = newLeftLayer;
            currentMap.addLayer(leftLayer);
    ...
    document.getElementById('monthRight').addEventListener('change', function() {
        ...
        const newRightLayer = Layersmonth[`AOD ${selectedMonthRight}`];
        if (newRightLayer) {
            if (rightLayer) currentMap.removeLayer(rightLayer);
            rightLayer = newRightLayer;
            currentMap.addLayer(rightLayer);
    ...
    const overlayLayers = {
        "AOD Year": YearLayer,
        "AOD Month": MonthLayer,
        "AOD Trend" : await map_trend(currentMap)
    };
    ...
    currentMap.on('overlayadd', function(event) {
        ...
        } else if (event.name === "AOD Trend") {
            // Mostrar la leyenda de tendencia cuando se seleccione la capa AOD Trend
    ...
    currentMap.on('overlayremove', function(event) {
        if (event.name === "AOD Year") {
            ...
            // Eliminar la leyenda de tendencia cuando se deseleccione la capa AOD Trend
            if (legendDiv) {
                legendDiv.innerHTML = '';
            }
        }
