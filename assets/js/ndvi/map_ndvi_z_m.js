import { createYearSelector, positionYearSelector } from './ndvi_z_m/utils_z_m_y.js';
import { createMonthSelector, positionMonthSelector } from './ndvi_z_m/utils_z_m_m.js';
import { LayersControl } from '../control.js';
import { updateMapLayerYear, updateMapLayerMonth } from './ndvi_z_m/layer_z_m.js';
import { createYearLegend, createMonthLegend } from './ndvi_z_m/legend.js';
import {map_trend, createTrendLegend} from './ndvi_z_m/trend_m.js';
import{ loadInfCriticaMapLibre } from '../inf_critica_map_libre.js';

export async function map_ndvi_zonal_m() {

  const container = document.getElementById('p07_m');
  container.innerHTML = `
    <div id="before_manzanas" style="width: 100%; height: 100%;"></div>
    <div id="after_manzanas" style="width: 100%; height: 100%;"></div>
<div id="title" class="map-title">
    NDVI Estadística Zonal (Manzana)
    </div> 
  `; 
...
const afterMap = new maplibregl.Map({
   ...
   container.appendChild(trendLegend);
    ...
    function createOpacitySlider() {
      ...
      // Agregar los estilos CSS para el slider (idénticos al otro archivo)
    ...
    window.compareInstance = new maplibregl.Compare(beforeMap, afterMap, '#p07_m', {
      mousemove: false,
