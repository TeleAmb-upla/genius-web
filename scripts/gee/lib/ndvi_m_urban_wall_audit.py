"""
Auditoría local de ``NDVI_m_urban.csv``: columna ``anio_actual`` para meses 1..último mes
...
Sirve tras ``download_drive_to_repo`` sin inicializar Earth Engine. Uso:
    ...
    python -m scripts.gee.lib.ndvi_m_urban_wall_audit
"""
...
def _cell_valid_anio_ndvi(raw: str) -> bool:
    s = (raw or "").strip()
    if not s:
        return False
    try:
...
def audit_ndvi_m_urban_repo_csv(
    path: Path,
    *,
    ...
    Returns:
        (ok, líneas de texto para consola; vacío si ok).
    """
    ly, lm = wall if wall is not None else last_complete_calendar_month_utc_py()
    lines: list[str] = []
    if not path.is_file():
        lines.append(f"No existe {path} — sincronice CSV desde Drive.")
        return False, lines
    try:
        text = path.read_text(encoding="utf-8", errors="replace").strip()
    except OSError as e:
        lines.append(f"No se pudo leer {path}: {e}")
        return False, lines
    if not text:
        lines.append(f"{path.name} está vacío.")
        return False, lines
    try:
        reader = csv.DictReader(io.StringIO(text))
        fieldnames = reader.fieldnames or []
    except Exception as e:
        lines.append(f"CSV inválido: {e}")
        return False, lines
    if "Month" not in fieldnames or "anio_actual" not in fieldnames:
        lines.append(
            f"{path.name}: faltan columnas Month y/o anio_actual "
            f"(cabeceras: {fieldnames})."
        )
        return False, lines
    by_m: dict[int, str] = {}
    for row in reader:
    ...
    for m in range(1, lm + 1):
        if not _cell_valid_anio_ndvi(by_m.get(m, "")):
            bad.append(m)
    if not bad:
        return True, []
    lines.append(
        f"NDVI mensual urbano: ``anio_actual`` inválido o ausente para el año civil {ly} "
    ...
    lines.append(
        ...
        "Reexporte ``NDVI_m_urban`` (GEE) y vuelva a sincronizar; si persiste, "
        "revise que el asset año–mes NDVI incluya esos (año, mes)."
    )
    return False, lines


def ndvi_m_urban_anio_valid_for_wall_months(path: Path) -> bool:
    ok, _ = audit_ndvi_m_urban_repo_csv(path)
    return ok


def main(argv: list[str] | None = None) -> int:
    repo = Path(__file__).resolve().parents[3]
    csv_path = Path(repo) / "assets" / "data" / "csv" / "NDVI_m_urban.csv"
    if argv and len(argv) > 1:
        csv_path = Path(argv[1]).expanduser()
    ok, lines = audit_ndvi_m_urban_repo_csv(csv_path)
    for line in lines:
