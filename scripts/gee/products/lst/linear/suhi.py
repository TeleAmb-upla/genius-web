...
Leer de arriba abajo (estilo Earth Engine Code Editor). API estable vía ``../suhi_tasks.py``.
"""
...
def _suhi_geojson_for_year(
    ...
    d = ee.Dictionary(pointstat)
    names = d.keys()
    baseline_raw = ee.Algorithms.If(
        names.contains("LST_mean_median"),
        d.get("LST_mean_median"),
        ee.Algorithms.If(
            names.contains("median"),
            d.get("median"),
            ee.Algorithms.If(names.contains("LST_mean"), d.get("LST_mean"), None),
        ),
    )
    baseline = ee.Number(baseline_raw)
