...
export async function map_trend(map) {
    try {
        // Leer el archivo  
        const response = await fetch(resolveAssetUrl('assets/data/raster/AOD/AOD_Trend/AOD_Yearly_Trend.tif'));
...
export function createSTLegendSVG()  {
    ...
    const valueLabels = Array.from({ length: steps }, (_, i) => {
        ...
        if (i === 0) {
            ...
            return `<text x="60" y="${yPosition}" font-size="10" font-family="Arial">&gt;38.00</text>`; // Último bloque para >7.00
