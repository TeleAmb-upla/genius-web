...
function valueToSTColor(value) {
    const spec = trendLegendSpec('no2', 'zonalBarrio');
...
export async function map_trend(map) {
    const geojsonUrl = resolveAssetUrl('assets/data/geojson/NO2/NO2_Yearly_ZonalStats/NO2_Yearly_ZonalStats_Barrios/Trend_NO2_ZonalStats_Barrios.geojson');
    ...
    map.on('click', 'generic-trend-layer', (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(atmBarrioPopupHtmlTrend('no2', properties));
...
export function createTrendLegend() {
    ...
    title.innerHTML = `Tendencia NO<sub>2</sub> Barrios${geniusYearSpanSuffix('no2')}`;
    fillZonalTrendLegendPanel(legendContent, title, 'no2', 'zonalBarrio', {
        steps: 7,
        decimals: 2,
    });
