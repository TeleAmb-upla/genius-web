...
function valueToSTColor(value) {
    const spec = trendLegendSpec('aod', 'zonalBarrio');
    if (!spec) return 'rgba(0,0,0,0)';
    return trendColorFromValue(value, spec) ?? 'rgba(0,0,0,0)';
}

export async function map_trend(map) {
    const geojsonUrl = resolveAssetUrl('assets/data/geojson/AOD/AOD_Yearly_ZonalStats/AOD_Yearly_ZonalStats_Barrios/Trend_AOD_ZonalStats_Barrios.geojson');
    ...
    map.on('click', 'generic-trend-layer', (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(atmBarrioPopupHtmlTrend('aod', properties));
...
export function createTrendLegend() {
    ...
    title.textContent = geniusTitleForProduct('Tendencia AOD Barrios', 'aod');
    title.className = 'map-legend-panel__title';
    fillZonalTrendLegendPanel(legendContent, title, 'aod', 'zonalBarrio', {
        steps: 7,
        decimals: 2,
    });
