...
import { ndviManzanaPopupHtmlTrend } from '../ndvi_zonal_explorer.js';
...
export async function map_trend(map) {
    ...
    const domain = legendDomain('ndvi', 'zonalManzana', 'trend');
    ...
    map.on('click', 'generic-trend-layer', (e) => {
        ...
        const popup = new maplibregl.Popup({ className: 'geo-popup' })
            .setLngLat(e.lngLat)
            .setHTML(ndviManzanaPopupHtmlTrend(properties));
...
export function createTrendLegend() {
    ...
    title.textContent = geniusTitleForProduct('Tendencia NDVI Manzanas', 'ndvi');
    title.className = 'map-legend-panel__title';
    fillZonalTrendLegendPanel(legendContent, title, 'ndvi', 'zonalManzana', {
        steps: 7,
        decimals: 2,
    });
