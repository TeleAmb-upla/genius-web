...
const afterMap = new maplibregl.Map({
    container: 'after_b_so2', // Contenedor ajustado a 'after_b_so2'
    style: {
        version: 8,
        sources: {
            carto: {
    ...
    // Mostrar/ocultar los selectores y leyendas según el modo
      if (mode === 'trend') {
        ...
        setCompareSingleMapMode({
            container,
            beforeSelector: '#before_b_so2',
            afterSelector: '#after_b_so2',
            beforeMap,
            afterMap,
            enabled: true,
        });
        ...
        if (!window.compareInstance) {
          window.compareInstance = new maplibregl.Compare(beforeMap, afterMap, '#p40', {
            mousemove: false,
